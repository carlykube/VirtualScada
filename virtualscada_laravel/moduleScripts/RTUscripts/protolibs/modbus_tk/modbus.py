...
class Master:
    ...
    def execute(self, slave, function_code, starting_address,
        ...
        if function_code == defines.READ_COILS or function_code == defines.READ_DISCRETE_INPUTS:
            ...
            raise ModbusFunctionNotSupportedError("The %s function code is not supported. " % str(function_code))
        ...
        if slave != 0:
            ...
            response = self._recv(expected_length, request[:1])
...
class ModbusBlock:
    ...
    def __setitem__(self, r, v):
        ...
        try:
            ...
            else: #This happens if item isn't iterable:
                if (str(type(item)) == "<class 'point.Point'>"
                    or str(type(item)) == "<class 'point.PointView'>" ):
                    item = item.set(v)
                else:
                    return self._data.__setitem__(r, v)
        finally: #Check that the set actually worked
            if hasattr(v,'__iter__') and (self.__getitem__(r) == list(v)):
                pass
            elif ((str(type(v)) == "<class 'point.Point'>") 
              or (str(type(v)) == "<class 'point.PointView'>") 
              and self.__getitem__(r) == v.get() ):
                pass
